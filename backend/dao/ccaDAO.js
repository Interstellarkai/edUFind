	...
	static async injectDB(conn) {
		...
		try {
			CCAs = await conn
				...
				.collection("co_curriculum_activities");
		} catch (e) {
			console.error(
	...
	static async getCCAs({
		...
		if (filters) {
			...
			if ("cca_grouping_desc" in filters) {
				query["cca_grouping_desc"] = {
					$eq: filters["cca_grouping_desc"],
				};
			}
			if ("cca_generic_name" in filters) {
				query["cca_generic_name"] = {
					$eq: filters["cca_generic_name"],
				};
			}
		}
		...
		// .limit(CCAsPerPage)
		// .skip(CCAsPerPage * page) // get to a specific page
