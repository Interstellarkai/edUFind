...
export default class UserController {
    static async registerUser(req, res, next) {
        ...
        if (!username || !email || !password) {
            ...
            return res.json({ success: false, message: 'Compulsory fields (Username, Email, Password) are not completed' })
        }
        ...
        if (!email.match(emailRegex)) {
            ...
            return res.json({ success: false, message: 'Email must be in name@email.com format' });
        ...
        if (password.length < 6 || password.lenth > 50){
            ...
            return res.json({ success: false, message: 'Password should be within length of 6 - 50.' });
        }
        try {
    ...
    static async editAccountDetails(req, res){
        try {
            ...
            if (ReviewResponse.modifiedCount == 0) {
                return res.json({ success: false, message: "Unable to edit account details" });
            }
